...
newt <- function(theta, func, grad, hess = NULL, ..., tol = 1e-8, fscale = 1,   maxit = 100, max.half = 20, eps = 1e-6){
  ...
  while(i <= maxit){
    ...
    while(obj_at_theta < func(theta_hat) && halves <= max.half){
      ...
      theta_hat <- theta +  stepsize * descent_direction
      halves <- halves + 1
      if (func(theta_hat) == -Inf){
        warning("Optimal objective function value is negative infinity.")
        return(list('f' = -Inf, 'theta' = original_theta, 'iter' = i, 'g' = "Undefined gradient.", 'Hi' = inv_hess))
      }
    }
