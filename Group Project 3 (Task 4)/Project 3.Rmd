...
newt <- function(theta, func, grad, hess = NULL, ..., tol = 1e-8, fscale = 1, maxit = 100, max.half = 20, eps = 1e-6){
  ...
  while(i <= maxit){
    ...
    if(sum(abs(grad_at_theta) < tol * abs(obj_at_theta) + fscale) == length(theta)){
      if(hess_has_inverse){
        return(list('f' = obj_at_theta, 'theta' = theta, 'iter' = maxit, 'g' = grad_at_theta, 'Hi' = inv_hess))
        
      }  
      warning('Hessian is not positive definite at convergence.')
      return(list('f' = obj_at_theta, 'theta' = theta, 'iter' = maxit, 'g' = grad_at_theta, 'Hi' = "Hessian is not invertible"))
