...
```{r}
split_conc <- strsplit(as.character(df3[1,1]), " +")
df4 <- data.frame(w1 = 1, w2 = 1, w3 = 1, Freq = df3[2])
for(i in 1:length(df3[[1]])){
  df4[i,1] <- result_list[as.numeric(strsplit(as.character(df3[i,1]), " +")[[1]][1])]
  df4[i,2] <- result_list[as.numeric(strsplit(as.character(df3[i,1]), " +")[[1]][2])]
  df4[i,3] <- result_list[as.numeric(strsplit(as.character(df3[i,1]), " +")[[1]][3])]
}
